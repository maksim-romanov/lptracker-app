...
- A colour that both fills something and is set as words needs two roles, not one compromise value — `primary`/`primaryText`, `success`/`successText`. Yellow is where skipping this is most visible: forced to carry white text it darkens into olive.
