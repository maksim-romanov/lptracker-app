...
const computeFeeGrowthInside = (
  ...
  return subInUint256(
    feeGrowthGlobalX128,
    feeGrowthOutsideLowerX128 + feeGrowthOutsideUpperX128
  )
}
...
export async function getPositionFees(tokenId: string): Promise<Response> {
  ...
  const multicallResults = await publicClient.multicall({
    contracts: [
      ...
      { address: networks.arbitrum.deployments.NonfungiblePositionManager, abi: NonfungiblePositionManagerABI.abi, functionName: 'positions', args: [BigInt(tokenId)] },
    ],
  })
  ...
  const feeGrowthInside0DeltaX128 = subInUint256(
    feeGrowthInside0X128,
    feeGrowthInside0LastX128
  )
  const feeGrowthInside1DeltaX128 = subInUint256(
