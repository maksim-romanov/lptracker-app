...
struct PositionsProvider: AppIntentTimelineProvider {
  ...
  func timeline(for configuration: SelectPositionIntent, in context: Context) async -> Timeline<PositionsEntry> {
  ...
  private func buildEntry(for configuration: SelectPositionIntent) -> PositionsEntry {
    ...
    let position = snapshot?.positions.first { $0.ref == configuration.position?.id }
